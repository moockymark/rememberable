...
namespace Moocky\Rememberable\Query;
...
class Builder extends \Illuminate\Database\Query\Builder
{
  ...
  public function aggregateCached($function, $columns = ['*'])
  {
    ...
    // for this database connection and query statement, including the bindings
    // that are used on this query, providing great convenience when caching.
    $key = $this->getCacheKey($function);
    $seconds = $this->cacheSeconds;

    $cache = $this->getCache();

    $callback = $this->aggregateCacheCallback($function, $columns);
   ...
   * @return \Closure
   */
  protected function aggregateCacheCallback($function, $columns)
  {
    return function () use ($function, $columns) {
      $this->cacheSeconds = null;
